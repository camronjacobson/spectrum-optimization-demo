...
def validate_dataframe_for_optimization(df):
    """
    Validate dataframe before optimization to prevent errors
    """
    logger.info("Validating data for optimization...")
    
    # Check required columns
    required_cols = ['x_coord', 'y_coord', 'bandwidth_mhz', 'station_id', 'cluster', 'area_type']
    ...
    if invalid_coords.any():
        logger.warning(f"Found {invalid_coords.sum()} stations with invalid coordinates")
        # Remove invalid rows
        df = df[~invalid_coords].copy()
        
    # Check for invalid bandwidth
    invalid_bw = (
        df['bandwidth_mhz'].isna() |
        ~np.isfinite(df['bandwidth_mhz']) |
        (df['bandwidth_mhz'] <= 0)
    )
    
    if invalid_bw.any():
        logger.warning(f"Found {invalid_bw.sum()} stations with invalid bandwidth")
        df = df[~invalid_bw].copy()
    
    if len(df) == 0:
        raise ValueError("No valid stations remaining after validation!")
    
    logger.info(f"✅ Validation complete: {len(df)} valid stations")
    return df
...
def load_data(filepath):
    ...
    # CRITICAL: Validate the data
    df = validate_dataframe_for_optimization(df)
...
def build_global_conflict_matrix(df, use_spatial_index=None):
    ...
    # CRITICAL FIX: Validate and clean data before processing
    logger.info(f"🔍 Validating data before building conflict matrix...")
    ...
    if invalid_mask.any():
        ...
        logger.warning(f"⚠️  Found {n_invalid} stations with invalid coordinates")
        logger.warning(f"Removing invalid stations from conflict analysis")
        
        # Create a clean dataframe without invalid coordinates
        df_clean = df[~invalid_mask].copy()
        
        # If all stations are invalid, return empty conflicts
        if len(df_clean) == 0:
            logger.error("❌ No valid stations remaining for conflict analysis!")
            return {}
            
        logger.info(f"Continuing with {len(df_clean)} valid stations")
    ...
    if use_spatial_index:
        ...
        # Additional validation before KDTree
        if not np.all(np.isfinite(coords)):
            logger.error("❌ Coordinates contain non-finite values after cleaning!")
            # Fallback to non-spatial method
            ...
            try:
                ...
                logger.error(f"❌ KDTree creation failed: {e}")
                logger.info("Falling back to pairwise comparison method")
                use_spatial_index = False
    
    if not use_spatial_index:
        # Fallback to pairwise comparison for smaller datasets or if KDTree fails
        ...
        for i in range(len(indices)):
            for j in range(i + 1, len(indices)):
                ...
                # Skip if coordinates are invalid (extra safety check)
                if (pd.isna(row1['x_coord']) or pd.isna(row1['y_coord']) or
                    pd.isna(row2['x_coord']) or pd.isna(row2['y_coord'])):
                    continue
...
def run_optimizer(input_csv, output_csv, use_partitioning=True,
    ...
    if result is not None:
        ...
        if plot_results:
            logger.info("\n📊 Visualization disabled to avoid generating multiple PNG files")
            # Commenting out to prevent PNG generation
            """
            for cluster_id in result["cluster"].unique():
                cluster_data = result[result["cluster"] == cluster_id]
                state = cluster_id.split("_")[0]
                
                if state in license_bands and cluster_data["allocation_method"].iloc[0] == "Local_CP":
